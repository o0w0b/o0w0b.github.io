    ...
    function startExecutionPlayer() {
        ...
        function allPlayerFeatures() {
            ...
            function selectPlaylist() {
                var time = new Date()
                // var isNight = (time.getSeconds() >= 40 || time.getSeconds() < 20) // 测试的时候用的
                var isNight = (time.getHours() >= 18 || time.getHours() < 6); // 18:00 到次日 06:00 是夜晚
                isNight = debug ? !isNight : isNight
                var playlistKey = isNight ? 'night_playlist' : 'day_playlist'

                return playlistKey
            }

            // 加载并使用JSON文件中的播放列表
            async function loadLocalPlaylist(url) {
                // 根据时间选择相应的播放列表部分
                var playlistKey = selectPlaylist()

                try {
                    const res = await fetch(url)
                    const data = await res.json()
                    const selectedPlaylist = await data[playlistKey]
                ...
                const playBackAndForth = async () => {
                    try {
                        ...
                        if (interfaceAndLocal === null && xfSongList !== null) {
                            ...
                            res = await loadLocalPlaylist(musicUrl)
                        }
